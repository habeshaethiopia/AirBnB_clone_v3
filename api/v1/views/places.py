...
def list_places_of_city(city_id):
    ...
    if city_obj == []:
        abort(404)
    list_places = [
        obj.to_dict() for obj in storage.all("Place").
        values() if city_id == obj.city_id
    ]
...
def delete_place(place_id):
    ...
    all_places = storage.all("Place").values()
    place_obj = [obj.to_dict() for obj in
...
def create_place(city_id):
    ...
    user_obj = [obj.to_dict() for obj in
                all_users if obj.id == new_place.user_id]
    if user_obj == []:
        abort(404)
